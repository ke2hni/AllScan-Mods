...
install_allscan_if_missing() {
    ...
    required=(
        ...
        "$ALLSCAN_DIR/include/viewUtils.php"
        "$ALLSCAN_DIR/js/main.js"
        "$ALLSCAN_DIR/css/main.css"
        "$ALLSCAN_DIR/astapi/connect.php"
        "$ALLSCAN_DIR/include/common.php"
    )
    ...
    command -v apt-get >/dev/null 2>&1 || {
        echo "ERROR: apt-get is required to install AllScan automatically." >&2
        exit 1
    }
    command -v php >/dev/null 2>&1 || {
        echo "Installing PHP and AllScan prerequisites..."
        apt-get update
        apt-get install -y php php-sqlite3 php-curl unzip avahi-daemon asl3-tts
    }
    ...
    for required_file in "${required[@]}"; do
        [[ -f $required_file ]] || {
            echo "ERROR: Official AllScan installation did not create: $required_file" >&2
            exit 1
        }
    done
}
...
ensure_apache_sqlite() {
    command -v apt-get >/dev/null 2>&1 || {
        echo "ERROR: apt-get is required to configure PHP SQLite support." >&2
        exit 1
    }

    if ! dpkg-query -W -f='${Status}' php-sqlite3 2>/dev/null | grep -q 'install ok installed'; then
        echo "Installing PHP SQLite support..."
        apt-get update
        apt-get install -y php-sqlite3
    fi

    local apache_ini changed=false sqlite_ok=false
    shopt -s nullglob
    local apache_inis=(/etc/php/*/apache2/php.ini)
    shopt -u nullglob

    if ((${#apache_inis[@]} == 0)); then
        echo "ERROR: No Apache PHP configuration was found under /etc/php/*/apache2/." >&2
        exit 1
    fi

    for apache_ini in "${apache_inis[@]}"; do
        if command -v phpenmod >/dev/null 2>&1; then
            version=${apache_ini#/etc/php/}
            version=${version%%/*}
            phpenmod -v "$version" -s apache2 pdo_sqlite sqlite3
        else
            sed -i \
                -e 's/^;[[:space:]]*extension[[:space:]]*=[[:space:]]*pdo_sqlite[[:space:]]*$/extension=pdo_sqlite/' \
                -e 's/^;[[:space:]]*extension[[:space:]]*=[[:space:]]*sqlite3[[:space:]]*$/extension=sqlite3/' \
                "$apache_ini"
        fi

        if php -c "$apache_ini" -m 2>/dev/null | grep -qx 'pdo_sqlite' && \
           php -c "$apache_ini" -m 2>/dev/null | grep -qx 'sqlite3'; then
            sqlite_ok=true
        fi
    done

    if [[ $sqlite_ok != true ]]; then
        echo "Restarting Apache so PHP SQLite extensions become active..."
    ...
    [[ $sqlite_ok == true ]] || {
        echo "ERROR: PHP SQLite extensions are not enabled for Apache PHP." >&2
        echo "Required extensions: pdo_sqlite and sqlite3" >&2
        exit 1
    }
    echo "PASS: Apache PHP SQLite support verified (pdo_sqlite, sqlite3)."
}
